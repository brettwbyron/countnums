...
( function () {
	...
	function intersectionCallback( entries ) {
		for ( let i = 0; i < entries.length; i++ ) {
			...
			if ( entry.intersectionRatio >= 0 && entry.intersectionRatio <= 1 && !animated ) {
				countUp( entry, options );
			} else if ( entry.intersectionRatio > 1 || entry.intersectionRatio < 0 && !!onlyOnce && !animated ) {
				animated = true;
			} else {
				return;
			}
